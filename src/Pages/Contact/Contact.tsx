      ...
      <div>
        <Form ref={form} onSubmit={sendEmail}>
          <Form.Group controlId="formName">
            ...
            <Form.Control type="text" name="to_name" placeholder={t('contactUs.enterYourName')} value={Name} onChange={(e)=>setName(e.target.value)} />
          </Form.Group>
          <Form.Group controlId="formEmail">
            <Form.Label>{t('contactUs.email')}</Form.Label>
            <Form.Control type="email" name="from_name" placeholder={t('contactUs.enterYourEmail')} value={Email} onChange={(e)=>setEmail(e.target.value)}/>
          </Form.Group>
          <Form.Group controlId="formMessage">
            <Form.Label>{t('contactUs.message')}</Form.Label>
            <Form.Control as="textarea" rows={4} name="message" placeholder={t('contactUs.typeYourMessage')}value={Message} onChange={(e)=>setMessage(e.target.value)} />
          </Form.Group>
